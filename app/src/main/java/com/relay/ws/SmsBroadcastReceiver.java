...
public class SmsBroadcastReceiver extends BroadcastReceiver {
    ...
    protected void callWebHook(ForwardingConfig config, String sender, String slotName,
        ...
        ForwardNotifier.notify(this.context, sender, content);
        RequestWorker.enqueue(this.context, buildWebHookData(config, sender, slotName, content, timeStamp));
    }

    // Builds the worker input for one dispatch. Carries the rule key and sender so
    ...
    // Full per-config match used by onReceive (live SMS, SIM detected from
    // the bundle). Sender/enabled/filter checks read only the config.
